...
class BusinessRiskAgent(BaseAgent):
    ...
    def run(self, context: AgentContext) -> AgentContext:
        ...
        levels = {DIMENSION_LABELS.get(d["dimension"], d["dimension"]): LEVEL_LABELS.get(d["level"], d["level"]) for d in context["risk"]["dimensions"]}
        self.event(context["task_id"], "completed", f"经营风险与交付风险评估完成：{levels}。", [])
        return context
