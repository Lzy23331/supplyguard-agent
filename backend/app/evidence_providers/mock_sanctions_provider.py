...
class MockSanctionsProvider(EvidenceProvider):
    name = "MockSanctionsProvider"
    source_type = "mock_external"

    def __init__(self, path: Path | None = None) -> None:
        self.path = path or get_settings().project_root / "data" / "external_mock" / "sanctions_results.json"
    ...
    def search(self, supplier: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [
            {**row, "id": f"sanctions-{index}", "metadata_json": {"matched_company": row.get("company_name")}}
